...
class ArcdStorageHook(PathSimulatorHook):
    """
    arcd storage hook - save to arcd storage in predefined intervals
...
class DensityCollectionHook(PathSimulatorHook):
    ...
    # TODO: take model or density_collector directly
    #       I think we can leave the DC attached to a model, it is model specific anyway
    #       but maybe we do want the flexibilty here anyways?
    def __init__(self, model, first_collection=100, interval=10, recreate_interval=500):
        self.model = model
    ...
    def after_step(self, sim, step_number, step_info, state, results, hook_state):
        ...
        if step_number - self.first_collection >= 0:
            if step_number - self.first_collection == 0:
                # first collection
                tps, counts = self._get_tps_from_ops_storage(sim.storage,
...
class TrainingHook_old(PathSimulatorHook):
    # TODO: remove all unecessary / move to the two other hooks
    # TODO: any other hooks we need?
    # TODO: do we want a 'arcd_hooks_attach' function? -> I think we do!,
    #       It should take model, trainset and storage (some possibly None values) and attach only needed hooks
    """
    ...
    save_trainset_states = 'arcd.TrainSet.states'
    # descriptor_transform does not change during simulation either
    save_trainset_descriptor_transform = 'arcd.TrainSet.descriptor_transform'
    # whether we add invalid MCSteps to the TrainSet
    # this is passed to TrainSet.add_ops_step() as add_invalid
