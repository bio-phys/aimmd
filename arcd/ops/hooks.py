...
class ArcdStorageHook(PathSimulatorHook):
    """Save model and trainset to arcd storage in predefined intervals."""
    ...
    def __init__(self, storage, model, trainset, interval=500,
        ...
        Parameters
        ----------
        storage - :class:`arcd.Storage`, the storage to save to
        model - :class:`arcd.RCModel`, the arcd RCModel to save
        trainset - :class:`arcd.TrainSet`, the trainingset to save
...
class DensityCollectionHook(PathSimulatorHook):
    ...
    def __init__(self, model, first_collection=100, interval=10,
        ...
        Parameters
        ----------
        model - :class:`arcd.RCModel`, model for which the density collection
                will be done
...
class TrainingHook(PathSimulatorHook):
    ...
    def __init__(self, model, trainset, add_invalid_mcsteps=False):
        ...
        Parameters
        ----------
        model - :class:`arcd.RCModels`, the model to train
        trainset - :class:`arcd.TrainSet`, the trainingset used to store
                    shooting results
